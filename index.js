...
if (selection === "Quit"){
    ...
    const deptId = await db.query(`SELECT IFNULL ((SELECT id FROM department WHERE name = "${inqOutput.roleDepartment}"), "That department does not exist");`)
    rows = deptId[0]
    ...
    const mgrData = everyMgr[0].filter(manager => `${manager.first_name} ${manager.last_name}` == inqOutput.manager)
